...
Source-available or non-open source licenses are not eligible for approval on an exception basis under the [CNCF IP Policy]. However, certain technical interactions between a CNCF project and such packages may be permitted. For guidance on permissible interaction types, please see [Guidance for Interactions between CNCF Project Code and Closed Source / Source-Available Products].
